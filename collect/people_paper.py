# coding: utf-8
# @Time : 2021/12/19 16:39
# @Author : wowbat
# @File : people_paper.py
# @Describe: 人民日报数据采集
...
def get_paper_section(paper_url, section_list_selector):
    """
    获得报纸各部分信息
...
def get_section_news(section, paper_date, news_list_selector):
    """
    获得报纸各部分的新闻信息，包括网址和标题等
    :param section: 版面信息，含url和name，代表版面的url和名字
    :param paper_date: 报纸日期
    :param news_list_selector: 各版面的选择器，在config中进行定义
    :return:新闻信息列表
    """
    section_html = utils.get_url_content(section['url'])
    soup = BeautifulSoup(section_html, "lxml")
    news_node = soup.find(**news_list_selector)  # 通过选择器选择新闻列表
    news_list = []  # 新闻信息列表
    if news_node:
        news = news_node.find_all('a')  # 在列表中选择所有的链接
        for new in news:
            try:
                news_list.append(
                    {"date": paper_date,
                ...
                print("新闻：{}获取列表错误, 错误代码为：{}".format(new, e))
...
def analysis_new(new_url):
    ...
    subtitle_node = soup.find("h2")
    content_node = soup.find("div",id="ozoom")
    # 保存的信息
...
def get_all_news_by_date(giving_date):
    ...
    :return:
    """
    url, date_str = utils.get_paper_url(giving_date, type="人民日报")
    section_list = get_paper_section(url, config.People_Paper_Selectors['section_list_selector'])
    all_news_url_list = []
    for section in section_list:
        news_url_list = get_section_news(section, date_str,
                                         config.People_Paper_Selectors['news_list_selector'])  # 获得news列表
...
def save_detail_new_by_date(new_model, giving_date):
    ...
    print("《人民日报》({})保存完毕, 共保存{}条新闻".format(giving_date, len(unique_news_url_list)))


def collect_people_paper(start_date, collect_num):
    """
    ...
    :return:
    """
    new_model = News(source="人民日报")  # 存储模型
...
if __name__ == "__main__":
    start_date = datetime.date(2021, 12, 18)  # 给定日期
    collect_people_paper(start_date, 1)