...
def save_detail_new_by_date(new_model, giving_date):
    ...
    # 获得当天新闻对应的json路径
    json_url, date_str = utils.get_paper_url(giving_date, type="国防报")
    ...
    print("《国防报》({})保存完毕, 共保存{}条新闻".format(giving_date, len(exist_titles)))


def collect_defense_paper(start_date, collect_num):
    new_model = News(source="国防报")  # 存储模型
...
if __name__ == "__main__":
    ...
    collect_defense_paper(start_date, 1)
