...
def get_all_news_by_date(giving_date):
    ...
    :return:
    """
    url, date_str = utils.get_paper_url(giving_date, type="每日新报")
    # 获取报纸各版面信息
    section_list = get_section_in_paper(url, config.Xin_Paper_Selectors['section_list_selector'])
    all_news_url_list = []  # 所有新闻列表
    # 遍历每个版块，将版块的新闻都放到所有新闻列表中
    for section in section_list:
        news_url_list = get_news_in_section(section, date_str,
                                            config.Xin_Paper_Selectors['news_list_selector'])  # 获得news列表
...
def save_detail_new_by_date(new_model, giving_date):
    ...
    print("《每日新报》({})保存完毕, 共保存{}条新闻".format(giving_date, len(unique_news_url_list)))


def collect_xin_paper(start_date, collect_num):
    new_model = News(source="每日新报")  # 存储模型
...
if __name__ == "__main__":
    ...
    collect_xin_paper(start_date, 3)
